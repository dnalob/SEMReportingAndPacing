...
<!-- GET NOTICES -->
        ...
        <?php
            if (isset($_GET['error'])) {
                echo '<p class="alert alert-danger">Couldn\'t find email!</p>';
            }
            elseif (isset($_GET['success'])) {
                echo '<p class="alert alert-success">Check your inbox! Click the link to reset your password.</p>';
            }
        ?> 
...
<!-- FORM: ENTER EMAIL TO HAVE RESET PASSWORD SENT -->
                
        <section class="row">
            <h1 class="text-center">Recover Password</h1>
            
            <div class="col-sm-offset-4 col-sm-4">
                <form action="includes/process_check_email.php" method="post" name="login_form">   
                    <br/>
                    ...
                    <br/>
                    <input id="rec-submit" type="submit"/>
                    <input class="btn btn-default center-block" type="button" value="Send" onclick="recformhash(this.form, this.form.email);" /> 
                </form>
            </div>
        </section>
    </div>
