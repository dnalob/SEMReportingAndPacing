...
while($row = mysqli_fetch_array($g2col))
          {
            ...
            $genIds= array();
            $genIds['SEIMid'] = $row['SEIMid'];
            $genIds['SEIMsheet'] = $row['SEIMsheet'];
            $genIds['t2'] = $row['t2'];
            $genIds['t3'] = $row['t3'];                
            $genIdsList[] = $genIds;
...
$worksheetTitle = $genIdsList[0]['SEIMid'];
$worksheetTab = $genIdsList[0]['SEIMsheet'];
...
$files = glob(dirname(__FILE__) . '/../Reports/SEIMReports/*'); // get all file names
...
function runAccountReport(AdWordsUser $user, $filePath, $accountNumber) {
...
function runCampaignReport(AdWordsUser $user, $filePath, $accountNumber) {
      // Load the service, so that the required classes are available.
      $user->LoadService('ReportDefinitionService', ADWORDS_VERSION);
      // Optional: Set clientCustomerId to get reports of your child accounts
      $user->SetClientCustomerId($accountNumber);
...
// BUDGET REPORT FUNCTION (NOT IN USE)



// function runBudgetReport(AdWordsUser $user, $filePath, $accountNumber) {
//         // Load the service, so that the required classes are available.
//           $user->LoadService('ReportDefinitionService', ADWORDS_VERSION);
//           // Optional: Set clientCustomerId to get reports of your child accounts
//           $user->SetClientCustomerId($accountNumber);
...
//           // Create selector.
//           $selector = new Selector();
//           $selector->fields = array('Amount');
          
          
//           // Create report definition.
//           $reportDefinition = new ReportDefinition();
//           $reportDefinition->selector = $selector;
//           $reportDefinition->reportName = 'Budget Performance Report #' . time();
...
for($y=2;$y<100;$y++) {
    ...
    $batchRequest->addEntry($cellFeed->createInsertionCell($y, $five, ""));
}
...
if (date('d') == 01) {
  $highRange = date('Y-m-') . sprintf('%02d', date('d'));
}
elseif (date('d') <= 10) {
  $zeroLead = sprintf('%02d', (date('d') - 01));
...
for ($row = 0; $row < $myCount; $row++) {
    ...
    try {
      ...
      $filePath = dirname(__FILE__) . '/../Reports/SEIMReports/' . $name . '.csv';

      // Run the example.
      runCampaignReport($user, $filePath, $accountNumber);
    } catch (Exception $e) {
...
// GO TO SLEEP FOR A MOMENT TO PREVENT OVERWHELMING WITH REQUESTS (CAMPAIGN REPORT)
    ...
    $lastPath = dirname(__FILE__) . "/../Reports/SEIMReports/" . $name . ".csv";
    ...
    try {
      ...
      // Download the report to a file in the same directory as the example.
      $filePath = dirname(__FILE__) . '/../Reports/SEIMReports/' . $name . 'Test.csv';

      // Run the example.
      runAccountReport($user, $filePath, $accountNumber);
    } catch (Exception $e) {
      printf("An error has occurred: %s\n", $e->getMessage());
    }   

    
    $blastPath = dirname(__FILE__) . "/../Reports/SEIMReports/" . $name . "Test.csv";


// GO TO SLEEP FOR A MOMENT UNTIL REPORT IS DOWNLOADED (ACCOUNT REPORT)


    while (!file_exists($blastPath)) {
    sleep(1);
    }
